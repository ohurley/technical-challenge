...
def solve(input):
    """
    Function to provide whether a customer request can be fulfilled or is impossible
    :param input: must contain 'input' param object containing 3 keys:
            colors: int
            customers: int
            demands: list of lists
    :return: str - "IMPOSSIBLE", if the customers' preferences cannot be satisfied;
            OR N space-separated integers, one for each color from 1 to N
    """
    problem = json.loads(input)
